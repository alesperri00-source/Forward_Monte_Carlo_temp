...
void kink_move(std::vector<Vector3i> &polymer, int site,int thread_num, long int m, double beta){
    if ((polymer[(site+2)%pol_length] != polymer[site]) && (polymer[(site+2)%pol_length] != 2*polymer[(site+1)%pol_length]-polymer[site])){
        ...
        if (accept_move(delta_E_other(polymer, site, pol_length, prop_move1,thread_num), beta, thread_num)==1 && check_boundary_rest(prop_move1)==1 &&
            ...
            //         total_contacts[thread_num][std::min(elem,(site+1)%pol_length)][std::max(elem,(site+1)%pol_length)] += m - contacts[thread_num][{std::min(elem,(site+1)%pol_length),std::max(elem,(site+1)%pol_length)}]; // step at which contact ends
...
void crankshaft_move(std::vector<Vector3i> &polymer, int site, int thread_num,long int m, double beta){
...
void loop_move(std::vector<Vector3i> &polymer, int site, int thread_num, long int m, double beta){
